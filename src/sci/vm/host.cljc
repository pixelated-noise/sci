  ...
  (-> #?(:clj (reduce (fn [heap ns-sym] (merge heap (make-ns-registry ns-sym)))
      ...
      (assoc 'clojure.core/comp
             {:val (fn sci-comp
              ...
              :meta {:name 'comp :doc #?(:clj (:doc (meta #'clojure.core/comp)) :cljs nil)}})
      (assoc 'clojure.core/partial
             {:val (fn sci-partial
              ...
              :meta {:name 'partial :doc #?(:clj (:doc (meta #'clojure.core/partial)) :cljs nil)}})
      ...
      (assoc 'clojure.core/juxt
             {:val (fn sci-juxt
              ...
              :meta {:name 'juxt :doc #?(:clj (:doc (meta #'clojure.core/juxt)) :cljs nil)}})
      ...
      (assoc 'clojure.core/complement
             {:val (fn sci-complement [f]
                     (with-meta
                       ...
                       {:sci/complement-f f}))
              :meta {:name 'complement :doc #?(:clj (:doc (meta #'clojure.core/complement)) :cljs nil)}})
      ...
      (assoc 'clojure.core/every-pred
             {:val (fn sci-every-pred
              ...
              :meta {:name 'every-pred :doc #?(:clj (:doc (meta #'clojure.core/every-pred)) :cljs nil)}})
      ...
      (assoc 'clojure.core/some-fn
             {:val (fn sci-some-fn
              ...
              :meta {:name 'some-fn :doc #?(:clj (:doc (meta #'clojure.core/some-fn)) :cljs nil)}})
                         ...
                         (fn [& args]
                           (if-let [e (find @mem args)]
                             ...
                             (let [ret (apply f args)]
                               ...
                               ret)))
                         {:sci/memoize-f f})))
              :meta {:name 'memoize :doc #?(:clj (:doc (meta #'clojure.core/memoize)) :cljs nil)}})
      ...
      (assoc 'clojure.core/fnil
             {:val (fn sci-fnil
              ...
              :meta {:name 'fnil :doc #?(:clj (:doc (meta #'clojure.core/fnil)) :cljs nil)}})
                ...
                {:val (let [sci-pr-str (fn sci-pr-str [x]
                                         (cond
                                           ...
                                           (let [type-obj (:type (clojure.core/meta x))]
                                             (clojure.core/str "#" (type-name type-obj)
                                          ...
                                          (let [methods (type-methods sci-type)]
                                            (if-let [toString-fn (get methods 'toString)]
                                              ...
                                              (if (:sci.impl/record (clojure.core/meta x))
                                                (clojure.core/str "#" (type-name sci-type)
                                                                  (clojure.core/pr-str (into (sorted-map) x)))
                                                (clojure.core/str "#object[" (type-name sci-type) "]"))))
      ...
      #?(:clj
         (assoc 'clojure.core/assert
                {:val (fn [form _env & _]
                 ...
                 :meta {:name 'assert :macro true :doc (:doc (meta #'clojure.core/assert))}
                 :macro? true
                 :host-macro? true})
         :cljs identity)))
...
(defn inverse-registry
  "Build an identity-based lookup from fn/var objects to qualified symbols.
   Returns a java.util.IdentityHashMap (JVM) or js/Map (CLJS) for O(1)
   identity-based lookups."
  [heap]
  (let [m #?(:clj (java.util.IdentityHashMap.) :cljs (js/Map.))]
    (doseq [[qualified-sym entry] heap]
      (let [v (:val entry)]
        (if (:macro? entry)
          ;; For macros, map the var object
          (when #?(:clj (var? v) :cljs false)
            #?(:clj (.put m v qualified-sym) :cljs (.set m v qualified-sym)))
