...
(defn safe-step
  "Execute one VM step with exception handling and dynamic binding sync.
   Returns the updated machine. Only steps if status is :running; otherwise
   returns the machine unchanged."
  [machine]
  (if (not= :running (:status machine))
    machine
